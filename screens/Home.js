...
function Home({ navigation }) {
    ...
    let searchItem = (e) => {
        setFilterText(e)
        // let search = myData.filter(x => x.title.toLowerCase().includes(e))
        let search = myData.filter(x => {
            return Object.keys(x).some(key =>
                x[key].toString().toLowerCase().includes(filterText.toString().toLowerCase()))
        })
